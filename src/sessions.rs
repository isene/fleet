...
pub enum State {
    ...
    Parked,  // set aside by the user; sorts first, drops when work starts
}
...
impl State {
    ...
    fn rank(self) -> u8 {
        match self {
            // Parked sits at the top on purpose: a short block that never
            // moves, so the eye learns to start below it.
            State::Parked => 0,
            State::Capped => 1,
            State::Yours => 2,
            State::Working => 3,
